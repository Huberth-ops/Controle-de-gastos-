...
export default function App({ user, onSignOut }) {
  ...
  const [settings, setSettings] = useState({ familyName: "Minha Família", name1: "Parceiro 1", name2: "Parceiro 2" });
  ...
  const [editingExpense, setEditingExpense] = useState(null); // {id, name, value}
  ...
  useEffect(() => {
    setLoading(true);
    const ref = doc(db, "users", uid, "months", key);
    const unsub = onSnapshot(ref, (snap) => {
      if (snap.exists()) {
        setMonthData(snap.data());
      } else {
        setMonthData({ salary: 0, incomes: [], expenses: [] });
      }
      setLoading(false);
    });
    return () => unsub();
  ...
  useEffect(() => {
    ...
    for (let m = 0; m < 12; m++) {
      ...
      const ref = doc(db, "users", uid, "months", k);
      const unsub = onSnapshot(ref, (snap) => {
  ...
  async function saveMonth(data) {
    setSaving(true);
    try { await setDoc(doc(db, "users", uid, "months", key), data); }
    finally { setSaving(false); }
  }
  async function saveGoals(list) { await setDoc(doc(db, "users", uid, "settings", "goals"), { list }); }
  async function saveSettings(s) { await setDoc(doc(db, "users", uid, "settings", "app"), s); }
  ...
  function saveEditExpense() {
    if (!editingExpense?.name || !editingExpense?.value) return;
    const updated = { ...monthData, expenses: monthData.expenses.map(e => e.id === editingExpense.id ? { ...e, name: editingExpense.name.toUpperCase().trim(), value: Number(editingExpense.value) } : e) };
    setMonthData(updated); saveMonth(updated); setEditingExpense(null);
  }
  ...
  return (
    <div style={{ background:"#f8f7f4", minHeight:"100vh", maxWidth:480, margin:"0 auto", paddingBottom:90, fontFamily:"'Instrument Sans', 'DM Sans', sans-serif", color:"#1c1917" }}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <div className="hdr-family">
              Controle de <span>Finanças</span>
            </div>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6,marginTop:4}}>
            ...
            <button onClick={onSignOut} style={{background:"none",border:"none",fontSize:11,color:"#a8a29e",cursor:"pointer",textDecoration:"underline",padding:0}}>Sair</button>
          </div>
        </div>
      </div>
        ...
        <div className="card">
          ...
          {expenses.map(e => (
            <div key={e.id} className="exp-row">
              ...
              <button className="del-x" style={{color:"#a8a29e",fontSize:13}} onClick={() => setEditingExpense({id:e.id,name:e.name,value:String(e.value)})}>✏️</button>
      ...
      {/* MODAL EDITAR CONTA */}
      {editingExpense && (
        <div className="modal-bg" onClick={() => setEditingExpense(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-title">
              Editar conta
              <button className="btn btn-ghost btn-sm" onClick={() => setEditingExpense(null)}>✕</button>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              <div>
                <div style={{fontSize:11,color:"#a8a29e",textTransform:"uppercase",letterSpacing:"1px",fontWeight:700,marginBottom:5}}>Nome</div>
              ...
              <div>
                <div style={{fontSize:11,color:"#a8a29e",textTransform:"uppercase",letterSpacing:"1px",fontWeight:700,marginBottom:5}}>Valor (R$)</div>
                <input className="field" type="number" value={editingExpense.value}
                  onChange={e => setEditingExpense(p => ({...p, value: e.target.value}))}
                  onKeyDown={e => e.key==="Enter" && saveEditExpense()} />
              </div>
              <button className="btn btn-dark" style={{marginTop:4}} onClick={saveEditExpense}>Salvar</button>
            </div>
          </div>
        </div>
      )}
